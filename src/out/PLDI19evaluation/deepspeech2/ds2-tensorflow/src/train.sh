...
CUDA_VISIBLE_DEVICES=3 python3 xilun_deepSpeech_train.py --batch_size 32 --no-shuffle --max_steps 892 --num_rnn_layers 3 --rnn_type unidirectional --num_hidden 1024 --num_filters 32 --initial_lr 5e-4 --train_dir $model_dir --data_dir $data_dir --debug ${debug} --nchw ${nchw} --engine ${engine} --dummy ${dummy} #--log_device_placement True
